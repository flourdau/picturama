...
export default class MyPicturama {
	...
	myCenterY() {
		...
		if (window.myList.length > 0) MyPicturama.myChangeBg(URL.createObjectURL(window.myList[window.myPositionY][window.myPositionX]));
	}
			...
			for (let column = 0; column < window.myList[line].length; column++) {
				...
				reader.addEventListener(
					...
					() => {
						...
						if (column === 0 && line === 0) MyPicturama.myChangeBg(URL.createObjectURL(window.myList[0][0]));
					},
					false
				);
	...
	myLeft() {
		if (window.myPositionX === 0) window.myPositionX = window.myList[window.myPositionY--].length - 1;
		else window.myPositionX--;

		if (window.myPositionY < 0) window.myPositionY = window.myList.length - 1;
	...
	myRight() {
		...
		if (window.myPositionY > window.myList.length - 1) window.myPositionY = 0;
	 ...
	 **/
	updateUI() {
		if (this.page === 0) window.prevButton.setAttribute("hidden", "hidden");
		else window.prevButton.removeAttribute("hidden");

		if (this.page === this.pages - 1) window.nextButton.setAttribute("hidden", "hidden");
		else window.nextButton.removeAttribute("hidden");
	}
