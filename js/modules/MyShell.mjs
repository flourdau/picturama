...
export default class MyShell {
	constructor() {
		...
		// MESSAGE
		document.querySelector("#myMessage .my-shell-prompt input").addEventListener("input", function () {
		...
		for (let i = 0; i < buttonClose.length; i++) {
			buttonClose[i].addEventListener("click", function () {
				if (confirm("close?")) this.parentNode.parentNode.parentNode.classList.toggle("d-none");
			});
		}
		...
		for (let i = 0; i < buttonMax.length; i++) {
			buttonMax[i].addEventListener("click", function () {
				...
				if (this.parentNode.parentNode.parentNode.classList.contains("my-size-max")) document.querySelector("#myShells").style.padding = "0 1rem";
				else if (!this.parentNode.parentNode.parentNode.classList.contains("my-size-max") && window.innerWidth <= 700) document.querySelector("#myShells").style.padding = "1rem 1rem";
				else document.querySelector("#myShells").style.padding = "2rem 1rem";
			});
		}
		...
		function myNameButton(elmnt) {
			...
			if (elmnt.querySelector(".my-shell-name")) elmnt.querySelector(".my-shell-name").onmousedown = dragMouseDown;
			if (elmnt.querySelector(".my-shell-content")) elmnt.querySelector(".my-shell-content").onmousedown = dragMouseDown;
		...
		for (let i = 0; i < buttonNameDraggable.length; i++) {
			buttonNameDraggable[i].addEventListener("mouseover", function () {
				myNameButton(this);
			});
		}
	}
